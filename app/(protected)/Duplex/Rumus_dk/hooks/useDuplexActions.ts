// app/(protected)/Duplex/Rumus_dk/hooks/useDuplexActions.ts

import { useState } from 'react'
import axios from '@/lib/axios'
import Swal from 'sweetalert2'
import type { ApiResponse, DuplexApiItem, DuplexDataDK, FormData, SheetSizeItem } from '../types/types'
import { buildSheetLabel, extractErrorMessage, isDuplicateError } from '../lib/utils'

const swalSuccess = (text: string) =>
  Swal.fire({ icon: 'success', title: 'Berhasil!', text, timer: 1500, showConfirmButton: false })

const swalDuplicate = () =>
  Swal.fire({
    icon: 'warning',
    title: 'Data Sudah Ada!',
    html: `<p class="text-gray-600">Kombinasi <strong>GSM</strong> dan <strong>Ukuran</strong> sudah terdaftar.</p>
           <p class="text-sm text-gray-400 mt-2">Pilih kombinasi lain atau edit data yang sudah ada.</p>`,
    confirmButtonColor: '#f59e0b',
    confirmButtonText: 'Mengerti',
  })

const swalError = (err: unknown) =>
  Swal.fire({ icon: 'error', title: 'Error!', text: extractErrorMessage(err), confirmButtonColor: '#3b82f6' })

interface UseDuplexActionsProps {
  dataDK: DuplexDataDK[]
  sheetSizeList: SheetSizeItem[]
  refetch: () => Promise<void>
  setShowAddModal: (v: boolean) => void
  setShowEditModal: (v: boolean) => void
  resetAdd: () => void
  resetEdit: () => void
  setEditingItem: (item: DuplexDataDK | null) => void
}

export const useDuplexActions = ({
  dataDK,
  sheetSizeList,
  refetch,
  setShowAddModal,
  setShowEditModal,
  resetAdd,
  resetEdit,
  setEditingItem,
}: UseDuplexActionsProps) => {
  const [isPosting, setIsPosting] = useState(false)

  // ===== VALIDATION =====
  const validate = (form: FormData, isEdit = false): Record<string, string> => {
    const errors: Record<string, string> = {}

    if (!form.gsm) errors.gsm = 'GSM tidak boleh kosong'
    if (!form.sheet_size_id) errors.sheet_size_id = 'Ukuran tidak boleh kosong'

    if (form.harga_per_lembar.trim()) {
      const h = parseFloat(form.harga_per_lembar)
      if (isNaN(h)) errors.harga_per_lembar = 'Harga harus berupa angka'
      else if (h < 0) errors.harga_per_lembar = 'Harga tidak boleh negatif'
    }

    if (!isEdit && form.gsm && form.sheet_size_id) {
      const gNum = parseInt(form.gsm)
      if (dataDK.some(d => d.sheet_size_id === form.sheet_size_id && d.gsm === gNum)) {
        const sz = sheetSizeList.find(s => s.id_sh === form.sheet_size_id)
        errors.general = `Kombinasi ${sz ? buildSheetLabel(sz.panjang_sh, sz.lebar_sh) : ''} dengan ${form.gsm} GSM sudah ada`
      }
    }

    return errors
  }

  // ===== ADD =====
  const handleAdd = async (form: FormData): Promise<Record<string, string> | null> => {
    const errors = validate(form, false)
    if (Object.keys(errors).length) return errors

    const payload = {
      gramasi: form.gsm,
      pl: form.sheet_size_id,
      harga_lembar: form.harga_per_lembar.trim() || '0',
    }

    try {
      setIsPosting(true)
      const res = await axios.post<ApiResponse>('Admin/Duplek/duplekKraftPricesAdd', payload)
      const raw = res.data as unknown

      if (typeof raw === 'string' && ((raw as string).includes('Duplicate') || (raw as string).includes('Database Error'))) {
        swalDuplicate()
        return null
      }

      if (res.status === 200 || res.data?.status === 200) {
        const harga = parseFloat(form.harga_per_lembar.trim() || '0')

        if (harga > 0) {
          await refetch()
          const freshRes = await axios.get<ApiResponse<DuplexApiItem[]>>('Admin/Duplek/duplekKraftPrices')
          const freshData: DuplexApiItem[] =
            freshRes.data?.status === 200 && Array.isArray(freshRes.data.data)
              ? freshRes.data.data
              : []
          const newItem = freshData.find(d => d.gsm === form.gsm && d.id_sh === form.sheet_size_id)
          if (newItem) {
            await axios.put<ApiResponse>(`Admin/Duplek/duplekKraftPricesEdit/${newItem.id}`, {
              gramasi: form.gsm,
              pl: form.sheet_size_id,
              harga_lembar: harga,
            })
          }
        }

        await refetch()
        await swalSuccess('Data Duplex DK berhasil ditambahkan')
        setShowAddModal(false)
        resetAdd()
      } else {
        throw new Error(res.data?.message || 'Gagal')
      }
    } catch (e) {
      if (isDuplicateError(e)) { swalDuplicate(); return null }
      swalError(e)
    } finally {
      setIsPosting(false)
    }

    return null
  }

  // ===== EDIT =====
  const handleEdit = async (item: DuplexDataDK, form: FormData): Promise<Record<string, string> | null> => {
    const errors = validate(form, true)
    if (Object.keys(errors).length) return errors

    const payload = {
      gramasi: form.gsm,
      pl: form.sheet_size_id,
      harga_lembar: parseFloat(form.harga_per_lembar || '0'),
    }

    try {
      setIsPosting(true)
      const res = await axios.put<ApiResponse>(`Admin/Duplek/duplekKraftPricesEdit/${item.id}`, payload)
      const raw = res.data as unknown

      if (typeof raw === 'string' && ((raw as string).includes('Duplicate') || (raw as string).includes('Database Error'))) {
        swalDuplicate()
        return null
      }

      if (res.status === 200 || res.data?.status === 200) {
        await refetch()
        await swalSuccess('Data Duplex DK berhasil diperbarui')
        setShowEditModal(false)
        setEditingItem(null)
        resetEdit()
      } else {
        throw new Error(res.data?.message || 'Gagal')
      }
    } catch (e) {
      if (isDuplicateError(e)) { swalDuplicate(); return null }
      swalError(e)
    } finally {
      setIsPosting(false)
    }

    return null
  }

  // ===== DELETE =====
  const handleDelete = async (id: number, gsm: number, ukuran: string) => {
    const ok = await Swal.fire({
      title: 'Konfirmasi Hapus',
      text: `Hapus data GSM ${gsm} - ${ukuran}?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#ef4444',
      confirmButtonText: 'Ya, Hapus!',
      cancelButtonText: 'Batal',
    })
    if (!ok.isConfirmed) return

    try {
      setIsPosting(true)
      const res = await axios.delete<ApiResponse>(`Admin/Duplek/duplekKraftPricesDel/${id}`)
      if (res.status === 200) {
        await refetch()
        await swalSuccess('Data berhasil dihapus')
      } else {
        throw new Error(res.data?.message || 'Gagal')
      }
    } catch (e) {
      swalError(e)
    } finally {
      setIsPosting(false)
    }
  }

  return { isPosting, validate, handleAdd, handleEdit, handleDelete }
}