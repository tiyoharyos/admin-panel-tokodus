...
export default function DuplexDKPage() {
    ...
    try {
      ...
      if (response.status === 200 || response.data?.status === 200) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: 'Data Duplex DK berhasil ditambahkan',
          timer: 1500,
    ...
    try {
      ...
      if (response.status === 200 || response.data?.status === 200) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: 'Data Duplex DK berhasil diperbarui',
          timer: 1500,
          showConfirmButton: false
        })
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Dihapus!',
          text: 'Data Duplex DK berhasil dihapus',
          timer: 1500,
          showConfirmButton: false
        })
      } else {
  ...
  // ===== VIEW HANDLER =====
  const handleViewClick = (item: DuplexDataDK) => {
        ...
        {[
          {
            ...
            sub: `${stats.uniqueGsm} variasi GSM`,
          },
          {
            icon: 'mdi:ruler-square',
            label: 'Kombinasi Ukuran',
            value: stats.totalCombinations,
            sub: `${stats.uniqueSizes} ukuran unik`,
          ...
          },
          {
            ...
            value: dataDK.filter(d => d.harga_per_lembar > 0).length,
            sub: `dari ${stats.totalRecords} data`,
            bar: (dataDK.filter(d => d.harga_per_lembar > 0).length / stats.totalRecords) * 100 || 0,
          },
        ].map((s, i) => (
        ...
        {sortedDataDK.length > 0 && (
          <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
            <p className="text-sm text-gray-500">
              Menampilkan <span className="font-medium text-slate-700">{sortedDataDK.length}</span> dari{' '}
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-2 bg-slate-50 rounded">
                    ...
                    <span className="text-sm font-bold text-slate-800">
                      {formatHargaDisplay(selectedItem.harga_per_lembar)}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700">DK (Duplex Kraft)</p>
                </div>
                <div>
