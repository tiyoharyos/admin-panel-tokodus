...
const LAYER_TYPES = [
  { value: 'K', label: 'Kraft (Coklat Tua)', badgeVariant: 'warning', badgeClass: 'bg-amber-100 text-amber-800' },
  { value: 'M', label: 'Medium (Coklat)', badgeVariant: 'default', badgeClass: 'bg-orange-100 text-orange-800' },
  { value: 'W', label: 'White (Putih)', badgeVariant: 'secondary', badgeClass: 'bg-gray-100 text-gray-800' }
]
...
export default function SheetSettingsPage() {
  ...
  const getLayerBadgeClass = (type: string): string => {
    switch (type) {
      case 'K': return 'bg-amber-100 text-amber-800'
      case 'M': return 'bg-orange-100 text-orange-800'
      case 'W': return 'bg-gray-100 text-gray-800'
  ...
  const fetchSheetIndex = useCallback(async (): Promise<SheetSubstance[]> => {
    try {
      ...
      setStats({
        totalSubstances: processedSubstances.length,
        activeSubstances: processedSubstances.length,
        withAllFlutes: processedSubstances.length,
                  ...
                  return (
                    <tr key={substance.id} className="hover:bg-blue-50 transition-colors duration-150">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-medium text-gray-900">{actualIndex}</div>
                      </td>
                      ...
                      {['layer_1', 'layer_2', 'layer_3'].map((layer) => {
                        const type = substance[`${layer}_type`]
                        return (
                          <td key={layer} className="px-6 py-4">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getLayerBadgeClass(type)}`}>
                              {substance[layer]}{type}
                            </span>
                          </td>
                        )
                      })}
                ...
                <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-blue-800">
                      Semua flute types wajib diisi. Harap isi harga untuk semua flute.
                    </p>
                  </div>
                </div>
                              ...
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (formErrors[`price_${fluteCode}`]) {
